...
gameController::gameController(int level, bool textOnly, int seed, string file1,
                               string file2)
    : level(level), textOnly{textOnly}, file1(file1), file2(file2) {
  if (seed >= 0) {
    srand(seed);
  }
  vector<string> blocks = {"I", "J", "L", "O", "S", "Z", "T"};
  // Assign blocks to the member variable
  this->blocks = blocks;
  ...
  command["norandom"] =
      "norandom"; // the next word we take in will be a filename
...
void gameController::run() {
  ...
  Potential problem for when renaming commands in map, keys are not able to be
  renamed
  ...
  */
  ...
  //         // Read in integers to add to it until hitting non-int using
  //         operator>>. cin >> lhs; delete sets[lhs]; sets[lhs] = new IntSet;
  //         cin >> *sets[lhs];
  //         break;
}

bool gameController::isABlock(string potentialBlock) {
  bool isInVecOfValidBlocks =
      std::find(blocks.begin(), blocks.end(), potentialBlock) != blocks.end();
...
void gameController::multipleCommmandHandler(string result) {
  if (result == "left") {
    ...
    // apply drop logic. should be done in a way such that i can continuously
    // call drop a ton of times and nothing happens
    cout << "drop" << endl;
...
//     cout << "Level:\t" << playerOne.level << "\t" << "Level:\t" <<
//     playerTwo.level << endl; cout << "Score:\t" << playerOne.score << "\t" <<
//     "Score:\t" << playerTwo.score << endl;
...
string gameController::fileParse(string fileName) {
  ...
  while (in >> s) {
    ...
    if (result == "") {
      ...
      temp = fileParse(temp);
      if (temp != "") {
        return temp; // only thing that should be returning should really just
                     // be "restart"
      }
    } else if (result == "norandom") { // norandom
      cout << "Set current player to norandom!" << endl;
    } else {
...
string gameController::decipherCommand(string toInterpret,
  ...
  // }
    ...
    if (readingFromFile) {
      ...
      cin >> fileName;
      return fileParse(
          fileName); // basically allows errors to propogate backwards
    }
  }
    ...
    // pass level seqFile. level knows that if seqFile isn't an empty string
    // then it has to use norandom from now on. otherwise it resets
  } else if (isABlock(result)) {
    cout << "We got a block!" << endl;
  } else if (result == "blind") {
    cout << "Blind time" << endl;
  } else if (result == "heavy") {
    cout << "heavy time" << endl;
  } else if (result == "force") {
    string block;
    cin >> block;
    if (isABlock(block)) {
      cout << "force taking place" << endl;
    }
  } else {
